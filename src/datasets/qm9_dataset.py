...
class QM9Dataset(InMemoryDataset):
    ...
    def __init__(self, stage, root, remove_h: bool, point_phore: list, transform=None, pre_transform=None, pre_filter=None):
        self.stage = stage
        if self.stage == 'train':
            self.file_idx = 0
        elif self.stage == 'val':
    ...
    def process(self):
        ...
        with open(self.raw_paths[1], 'r') as f:
            target = f.read().split('\n')[1:-1]
            target = [[float(x) for x in line.split(',')[1:24]]
                      for line in target]
        ...
        data_list = []
        for i, mol in enumerate(tqdm(suppl)):
            if i in skip or i not in target_df.index or mol is None or '.' in Chem.MolToSmiles(mol):
                continue
