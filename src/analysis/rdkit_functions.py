...
class BasicMolecularMetrics(object):
    def __init__(self, dataset_info, pp_graph_list, train_smiles=None):
        ...
        self.loaded_reg = joblib.load('/raid/yyw/PharmDiGress/data/stacking_regressor_model.pkl')
    ...
    def evaluate(self, generated):
        ...
        if relaxed_validity > 0:
            ...
            unique = []
        if len(valid) > 0:
            rdkit_mols = [Chem.MolFromSmiles(smiles) for smiles in valid]
            pharma_score = np.mean([max([match_score(mol, pp_graph) for pp_graph in self.pp_graph_list]) for mol in rdkit_mols])
            qed = np.mean([QED.default(mol) for mol in rdkit_mols])
            sa = np.mean([sascorer.calculateScore(mol) * 0.1 for mol in rdkit_mols])
            acute_tox = np.mean([self.loaded_reg.predict([AllChem.GetMorganFingerprintAsBitVect(mol, 2, 1024)]).item() for mol in rdkit_mols])
        else:
            pharma_score = 0
            qed = 0
            sa = 0
            acute_tox = 0
        return ([validity, relaxed_validity, uniqueness, novelty], unique, [pharma_score, qed, sa, acute_tox],
...
def compute_molecular_metrics(molecule_list, train_smiles, dataset_info):
    ...
    if not dataset_info.remove_h:
        ...
        for i, mol in enumerate(molecule_list):
            atom_types, edge_types, positions, rdkit_mol = mol
